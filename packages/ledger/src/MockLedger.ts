...
export default class MockLedger implements IBlockchain {
    ...
    public async read(
        ...
        if (this.hashes.length > 0 && sinceTransactionNumber < this.hashes.length - 1) {
            const transaction = {
                transactionNumber: hashIndex,
                transactionTime,
                transactionTimeHash: crypto.createHash('sha256').update(String(transactionTime)).digest('hex'),
